...
interface MentorAuthFormProps {
  isLogin: boolean;
  setIsLogin: (isLogin: boolean) => void;
}

const MentorAuthForm = ({ isLogin, setIsLogin }: MentorAuthFormProps) => {
      ...
      <Auth.Form gap="xxxl" align="center">
        <Stack gap="xl" align="center" width="100%">
          ...
          <Txt as="p" weight="light" color="secondary">
            가입이 완료되었습니다. 이제 로그인하여 멘토링을 시작하세요.
          </Txt>
  ...
  return (
    <Auth.Form gap='xxxl' align='center'>
      <form onSubmit={(e) => {
        e.preventDefault();
        console.log('Form submitted, isLogin:', isLogin); //TODO: 디버깅용
        if (isLogin) {
          onClickLogin(e as any);
        } else {
          onClickRegister(e as any);
        }
      }}>
        <Stack gap='xl' width='100%'>
        <Stack>
          <Txt as='h2' size='title' weight='regular'> 
            {isLogin ? '멘토 로그인' : '멘토 회원가입'} 
          </Txt>
          <Txt as='p' weight='light' color='secondary'> 
            {isLogin ? '학생들의 성장을 함께 관리하세요' : '블레이범에 오신 것을 환영합니다'} 
          </Txt>
        </Stack>
        <Stack gap='xl' align='center' width='100%'>
          ...
          <Button 
            ...
            type='submit'
            onClick={(event) => {
              console.log('Button clicked, isLogin:', isLogin, 'email:', email, 'password:', password); //TODO: 디버깅용
              if (isLogin) {
                onClickLogin(event);
              } else {
      ...
      <Auth.Info gap='lg'>
        ...
        <Stack gap='xs'>
          ...
          <Txt as='p' weight='light' color='secondary'> 
            학생 관리를 시작하려면 회원가입을 진행해주세요 
          </Txt>
          <Txt as='p' weight='light' color='highlight'> 
            멘토 지원팀에 문의하기
          </Txt>
        </Stack>
      </Auth.Info>
