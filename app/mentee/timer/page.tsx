...
export default function TimerPage() {
  ...
  const handleConfirmQuit = async () => {
    ...
    try {
      ...
      router.push(`/mentee/timer/complete?time=${elapsedTime}&pomoId=${finalPomoId ?? ""}`);
    } catch (error) {
      console.error("Failed to complete pomo session:", error);
      // Still navigate to complete page even on error
      router.push(`/mentee/timer/complete?time=${elapsedTime}&pomoId=${pomoId ?? ""}`);
    }
  };

  if (!isAuthed) {
    return <div className="min-h-screen flex items-center justify-center text-sm text-gray-500">로그인 확인 중...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF4] flex flex-col relative">
      ...
      <div className="flex items-center px-4 h-14">
        <button onClick={() => setShowQuitModal(true)} className="p-2 -ml-2 text-gray-400 hover:text-gray-600">
          <ChevronLeft size={24} />
        </button>
      ...
      {/* Quit Modal */}
      <QuitModal isOpen={showQuitModal} onClose={() => setShowQuitModal(false)} onConfirm={handleConfirmQuit} />
    </div>
  );
}
